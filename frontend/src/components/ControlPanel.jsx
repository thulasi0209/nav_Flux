...
const ControlPanel = ({ onStartAuto, onStopAuto, onStep, onSync, onUpdateParams, isAuto, isLoading }) => {
  const [time, setTime] = useState(new Date());
  const [drainRate, setDrainRate] = useState(1.0);
        ...
        <div className="flex flex-wrap gap-6 px-4 py-2 bg-black/20 rounded-lg border border-white/5">
           <div className="flex flex-col gap-1 min-w-[120px]">
              <div className="flex justify-between">
                ...
                <span className="text-[9px] text-neon-cyan opacity-80 font-bold">N=25</span>
              </div>
              <input
                type="range" min="1" max="25" defaultValue="8"
           ...
           <div className="flex flex-col gap-1 min-w-[100px]">
              <div className="flex justify-between">
                ...
                <span className="text-[9px] text-rose-400 opacity-80 font-bold">{drainRate.toFixed(1)}%/tick</span>
              </div>
              <input
                type="range" min="0.5" max="3" step="0.1" value={drainRate}
                onChange={(e) => setDrainRate(parseFloat(e.target.value))}
                onMouseUp={(e) => onUpdateParams({ drain_rate: parseFloat(e.target.value) })}
                className="w-full h-1 accent-rose-500 opacity-50 hover:opacity-100"
              />
           </div>
           <div className="flex flex-col gap-1 min-w-[100px]">
              <div className="flex justify-between">
                <span className="text-[9px] font-bold text-slate-500 uppercase">Charge Speed</span>
                <span className="text-[9px] text-emerald-400 opacity-80 font-bold">{chargeRate}%/tick</span>
              </div>
              <input
                type="range" min="5" max="20" step="1" value={chargeRate}
                onChange={(e) => setChargeRate(parseInt(e.target.value))}
                onMouseUp={(e) => onUpdateParams({ charge_rate: parseInt(e.target.value) })}
                className="w-full h-1 accent-emerald-500 opacity-50 hover:opacity-100"
              />
           </div>
        </div>
      </div>
