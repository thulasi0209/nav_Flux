...
const SimulationCanvas = ({ robots, gridSettings = { rows: 3, cols: 2 } }) => {
  ...
  useEffect(() => {
    ...
    const render = (time) => {
      ...
      // Draw Grid Background (density controlled by gridSettings.cols / .rows)
      ctx.strokeStyle = 'rgba(0, 242, 255, 0.03)';
      ctx.lineWidth = 1;
      const { rows: gridRows, cols: gridCols } = gridSettings;
      for (let i = 0; i <= gridCols; i++) {
        const gx = (i / gridCols) * 10;
        const { cx: xStart, cy: yStart } = getCanvasCoords(gx, 0, width, height);
        const { cx: xEnd, cy: yEnd } = getCanvasCoords(gx, 10, width, height);
        ctx.beginPath(); ctx.moveTo(xStart, yStart); ctx.lineTo(xEnd, yEnd); ctx.stroke();
      }
      for (let i = 0; i <= gridRows; i++) {
        const gy = (i / gridRows) * 10;
        const { cx: xStart2, cy: yStart2 } = getCanvasCoords(0, gy, width, height);
        const { cx: xEnd2, cy: yEnd2 } = getCanvasCoords(10, gy, width, height);
  ...
  }, [robots, robotPositions, gridSettings]);
