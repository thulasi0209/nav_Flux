...
import { API_URL, WS_URL, authHeaders } from './config';
...
function App() {
  ...
  useEffect(() => {
    if (wsData) {
      ...
      setHubs(wsData.hubs || []);
      if (wsData.metrics) {
        setMetrics(prev => ({ ...prev, ...wsData.metrics }));
      }
    }
  }, [wsData]);
  ...
  const handleStart = async () => {
    try {
      await fetch(`${API_URL}/simulate/start`, { method: 'POST', headers: authHeaders() });
  ...
  const handleReset = async (n) => {
    ...
    try {
      ...
      const resp = await fetch(`${API_URL}/simulate/reset?n=${targetN}`, { method: 'POST', headers: authHeaders() });
      if (resp.ok) {
  ...
  const handleUpdateParams = async (params) => {
    try {
      const resp = await fetch(`${API_URL}/simulate/params`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify(params)
      });
      if (resp.ok) {
        addToast('Battery Params Updated', 'info');
      }
    } catch (err) {
      addToast('Param Update Failed', 'error');
    }
  };
    ...
    <div className="relative min-h-screen">
      ...
      <main className="max-w-7xl mx-auto pt-32 px-6 pb-20 relative z-10">
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          ...
          <MetricCard label="Throughput" value={metrics?.throughput || 0} unit="GOALS/100T" icon="⚡" colorClass="border-emerald-500" />
